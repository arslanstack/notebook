...
<div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-lg-8 col-sm-8 col-xs-8">
        <h2> Media Catgeories </h2>
        <ol class="breadcrumb">
            <li class="breadcrumb-item">
                <a href="{{ url('admin') }}">Dashboard</a>
            </li>
            <li class="breadcrumb-item active">
                <strong> Media Catgeories </strong>
            </li>
        </ol>
    </div>
    <div class="col-lg-4 col-sm-4 col-xs-4 text-right">
        <a class="btn btn-primary text-white t_m_25" data-toggle="modal" data-target="#add_modalbox">
            <i class="fa fa-plus" aria-hidden="true"></i> Add New Media Category
        </a>
    </div>
</div>
        ...
        <div class="col-lg-12">
            <div class="ibox">
                <div class="ibox-content">
                    <form id="search_form" action="{{url('admin/media-categories')}}" method="GET" enctype="multipart/form-data">
                            ...
                            <tbody>
                                ...
                                <tr class="gradeX">
                                    ...
                                    <td>
                                        ...
                                        <button class="btn btn-danger btn-sm btn_delete" data-id="{{$item->id}}" data-text="you want to delete this media category?" type="button" data-placement="top" title="Delete">Delete</button>
                                    </td>
                                </tr>
                                @endforeach
                            </tbody>
                    ...
                    <div class="row">
                        <div class="col-md-9">
                            <p>Showing {{ $media_categories->firstItem() }} to {{ $media_categories->lastItem() }} of {{ $media_categories->total() }} entries</p>
                        </div>
                        <div class="col-md-3 text-right">
                            {{ $media_categories->links('pagination::bootstrap-4') }}
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="modal-dialog modal-md" role="document">
        <div class="modal-content animated flipInY">
            <div class="modal-header">
                ...
                <h5 class="modal-title">Add New Media Category</h5>
            </div>
...
<script>
        ...
        swal({
            ...
            function(isConfirm) {
                if (isConfirm) {
                    ...
                    $.ajax({
                        url: "{{ url('admin/media-categories/delete') }}",
                        type: 'post',
                        data: {
                            "_token": "{{ csrf_token() }}",
                            'id': id,
    ...
    $(document).on("click", "#save_category_button", function() {
        ...
        $.ajax({
            url: "{{ url('admin/media-categories/store') }}",
            type: 'POST',
            data: formData,
            dataType: 'json',
            cache: false,
    ...
    $(document).on("click", ".btn_category_edit", function() {
        var id = $(this).attr('data-id');
        $.ajax({
            url: "{{ url('admin/media-categories/show') }}",
            type: 'POST',
            dataType: 'json',
            data: {
                "_token": "{{ csrf_token() }}",
    ...
    $(document).on("click", "#update_category_button", function() {
        ...
        $.ajax({
            url: "{{ url('admin/media-categories/update') }}",
            type: 'POST',
            data: formData,
            dataType: 'json',
            cache: false,
